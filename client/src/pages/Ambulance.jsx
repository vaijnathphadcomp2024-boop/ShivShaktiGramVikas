...
function BookingForm() {
  const INIT = {
    name: '', phone: '', pickup: '', talukaDistrict: '',
    hospital: '', emergencyType: '', notes: '',
  };
  ...
  const validate = () => {
    ...
    if (!form.emergencyType)             e.emergencyType = 'Please select emergency type.';
    return e;
  };
  ...
  const handleSubmit = (e) => {
    ...
    setLoading(true);
    setTimeout(() => {
      // TODO: Wire up to POST /api/enquiries with formType: "Ambulance" in Module 13
      console.log('🚑 Ambulance Booking Request:', { ...form, formType: 'Ambulance' });
      setLoading(false);
      setSubmitted(true);
    }, 600);
  };
  ...
  const inputCls = (f) =>
    `w-full rounded-lg border px-4 py-3 text-sm text-gray-900 bg-white focus:outline-none focus:ring-2 transition-colors ${
      errors[f]
        ? 'border-red-400 focus:ring-red-300'
        : 'border-gray-300 focus:ring-[#e11d48]/30 focus:border-[#e11d48]'
    }`;

  if (submitted) {
    return (
      <div className="rounded-2xl bg-green-50 border-2 border-green-400 p-6 sm:p-8 text-center space-y-3" role="alert">
        <p className="text-4xl">✅</p>
        <p className="font-extrabold text-green-800 text-lg">Request Received!</p>
        <p className="text-green-700 text-sm leading-relaxed">
          Your request has been received. Our team will call you on{' '}
          <strong className="font-bold">{form.phone}</strong> shortly.
        </p>
        <p className="text-green-700 text-sm font-semibold">
          For faster response, call us directly:{'  '}
          <a href="tel:+919272418496" className="text-[#e11d48] font-extrabold underline">
            9272418496
          </a>
        </p>
        <button
    ...
    <form onSubmit={handleSubmit} noValidate className="space-y-4">
      <div className="grid sm:grid-cols-2 gap-4">
        ...
        <div className="flex flex-col gap-1">
          <label htmlFor="amb-name" className="text-sm font-semibold text-[#1a3c5e]">
            Full Name <span className="text-[#e11d48]">*</span>
          </label>
          <input
            id="amb-name" name="name" type="text"
        ...
        {/* Phone — auto-focus for emergency UX */}
        <div className="flex flex-col gap-1">
          <label htmlFor="amb-phone" className="text-sm font-semibold text-[#1a3c5e]">
            Mobile Number <span className="text-[#e11d48]">*</span>
          </label>
          <input
            id="amb-phone" name="phone" type="tel"
      ...
      {/* Pickup location */}
      <div className="flex flex-col gap-1">
        <label htmlFor="amb-pickup" className="text-sm font-semibold text-[#1a3c5e]">
          Pickup Village / Location <span className="text-[#e11d48]">*</span>
        </label>
        <input
          id="amb-pickup" name="pickup" type="text"
          placeholder="Village name, landmark, or address"
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        {/* Taluka / District */}
        <div className="flex flex-col gap-1">
          <label htmlFor="amb-taluka" className="text-sm font-semibold text-[#1a3c5e]">
            Taluka / District
          </label>
          <input
            id="amb-taluka" name="talukaDistrict" type="text"
            placeholder="e.g. Ahmadpur / Latur"
            value={form.talukaDistrict} onChange={handleChange}
            className={inputCls('talukaDistrict')}
          />
        </div>

        {/* Destination hospital */}
        <div className="flex flex-col gap-1">
          <label htmlFor="amb-hospital" className="text-sm font-semibold text-[#1a3c5e]">
            Destination Hospital <span className="text-gray-400 text-xs font-normal">(optional)</span>
          </label>
          <input
            id="amb-hospital" name="hospital" type="text"
            placeholder="Hospital name if known"
      ...
      {/* Emergency type */}
      <div className="flex flex-col gap-1">
        <label htmlFor="amb-type" className="text-sm font-semibold text-[#1a3c5e]">
          Emergency Type <span className="text-[#e11d48]">*</span>
        </label>
        <select
          id="amb-type" name="emergencyType"
          value={form.emergencyType} onChange={handleChange}
          className={inputCls('emergencyType')}
        >
          <option value="">— Select emergency type —</option>
          {EMERGENCY_TYPES.map((t) => <option key={t} value={t}>{t}</option>)}
        </select>
        {errors.emergencyType && <p className="text-xs text-red-600">{errors.emergencyType}</p>}
      </div>

      {/* Notes */}
      <div className="flex flex-col gap-1">
        <label htmlFor="amb-notes" className="text-sm font-semibold text-[#1a3c5e]">
          Additional Notes <span className="text-gray-400 text-xs font-normal">(optional)</span>
        </label>
        <textarea
          id="amb-notes" name="notes" rows={3}
          placeholder="Any additional information — patient condition, special requirements, etc."
      ...
      <button
        id="amb-request-submit"
        type="submit"
        disabled={loading}
        className="w-full py-4 rounded-xl bg-[#e11d48] hover:bg-rose-700 disabled:opacity-60 disabled:cursor-not-allowed text-white font-extrabold text-lg shadow-lg transition-colors duration-150 flex items-center justify-center gap-2"
        aria-label="Submit ambulance request"
      >
        {loading ? (
          <><svg className="w-5 h-5 animate-spin" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"/><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8H4z"/></svg> Submitting…</>
        ) : '🚑 Request Ambulance'}
      </button>
    </form>
  );
  ...
  return (
    <>
      ...
      <main>
        ...
        {/* ── 11. ONLINE BOOKING FORM ──────────────────────────────────────── */}
        <section id="booking-form" className="py-14 px-4 sm:px-6 lg:px-8 bg-white">
          <div className="max-w-2xl mx-auto">
            ...
            <p className="text-gray-500 text-sm mb-5">Request our ambulance online for advance bookings or non-urgent transport.</p>

            {/* Emergency warning — above form */}
            <div
              className="flex items-start gap-3 bg-red-50 border-2 border-[#e11d48] rounded-xl p-4 mb-6"
              role="alert"
              aria-live="assertive"
            >
              <span className="text-2xl shrink-0" aria-hidden="true">⚠️</span>
              <p className="text-[#e11d48] font-bold text-sm leading-relaxed">
                ...
                first. Use this form only for advance booking or non-urgent transport.
              </p>
            </div>

            <div className="bg-white border border-gray-100 rounded-2xl shadow-lg p-6 sm:p-8">
              <div className="h-1.5 w-20 rounded-full bg-[#e11d48] mb-6" />
              <BookingForm />
            </div>
          </div>
        </section>
