...
function JoinForm() {
  ...
  const handleSubmit = (e) => {
    ...
    setLoading(true);
    setTimeout(() => {
      console.log('📋 Read India Center — Join Form Submitted:', form);
      setLoading(false);
      setSubmitted(true);
      setForm(INIT);
    }, 800);
  ...
  const inputCls = (field) =>
    `w-full rounded-xl border px-4 py-3 text-sm text-gray-800 bg-white focus:outline-none focus:ring-2 transition-colors ${
      errors[field]
        ...
        : 'border-gray-200 focus:ring-cyan/40 focus:border-cyan'
    }`;

  if (submitted) {
    return (
      <div className="flex flex-col items-center py-12 gap-4 text-center">
        <div className="w-20 h-20 rounded-full bg-cyan/10 flex items-center justify-center text-4xl shadow">✅</div>
        <h3 className="text-2xl font-extrabold text-navy">Thank You!</h3>
        <p className="text-gray-500 max-w-sm text-sm">Your interest has been recorded. Our team will contact you within 24 hours.</p>
        <button
          onClick={() => setSubmitted(false)}
          className="mt-2 px-6 py-2.5 rounded-full bg-cyan hover:bg-[#0891b2] text-white font-semibold transition-colors"
        >
  ...
  return (
    <form onSubmit={handleSubmit} noValidate className="grid sm:grid-cols-2 gap-5">
      ...
      <div className="flex flex-col gap-1">
        <label htmlFor="ri-name" className="text-sm font-semibold text-navy">
          Full Name <span className="text-red-500">*</span>
        </label>
        <input id="ri-name" name="name" type="text" placeholder="Your full name"
      ...
      {/* Phone */}
      <div className="flex flex-col gap-1">
        <label htmlFor="ri-phone" className="text-sm font-semibold text-navy">
          Mobile Number <span className="text-red-500">*</span>
        </label>
        <input id="ri-phone" name="phone" type="tel" placeholder="10-digit mobile number"
          value={form.phone} onChange={handleChange} maxLength={10} className={inputCls('phone')} />
        {errors.phone && <p className="text-xs text-red-500">{errors.phone}</p>}
      </div>

      {/* Email */}
      <div className="flex flex-col gap-1">
        <label htmlFor="ri-email" className="text-sm font-semibold text-navy">
          Email Address <span className="text-red-500">*</span>
        </label>
        <input id="ri-email" name="email" type="email" placeholder="your@email.com"
      ...
      {/* Interest */}
      <div className="flex flex-col gap-1">
        <label htmlFor="ri-interest" className="text-sm font-semibold text-navy">
          I am interested in <span className="text-red-500">*</span>
        </label>
        <select id="ri-interest" name="interest" value={form.interest}
          onChange={handleChange} className={inputCls('interest')}>
      ...
      <div className="sm:col-span-2">
        <button
          ...
          type="submit"
          disabled={loading}
          className="w-full sm:w-auto px-10 py-3.5 rounded-full bg-cyan hover:bg-[#0891b2] disabled:opacity-60 disabled:cursor-not-allowed text-white font-bold text-base shadow-lg shadow-cyan/30 transition-all duration-200 hover:-translate-y-0.5 flex items-center gap-2"
        >
          {loading ? (
            <>
        ...
        <p className="mt-2 text-xs text-gray-400">* All fields are required. We respond within 24 hours.</p>
      </div>
    </form>
  );
}
...
export default function ReadIndia() {
  ...
  return (
    <main>
      ...
      {/* ── 9. Join / Volunteer / Partner Form ───────────────────────────── */}
      <section id="join" className="py-16 px-4 sm:px-6 lg:px-8 bg-white">
        <div className="max-w-3xl mx-auto">
          <SectionHeading
            tag="Get Involved"
            title="Join · Volunteer · Partner"
            subtitle="Whether you want to learn, teach, or support — there is a place for you at the Read India Center."
          />

          <div className="bg-white border border-gray-100 rounded-3xl shadow-xl p-6 sm:p-10">
            <div className="h-1.5 w-24 rounded-full bg-cyan mb-8" />
            <JoinForm />
          </div>

          {/* Contact alternative */}
          <div className="mt-8 flex flex-col sm:flex-row items-center justify-center gap-4">
            <p className="text-gray-400 text-sm">Prefer to visit in person?</p>
            <div className="flex gap-3">
              <a
                href="tel:+910000000000"
                className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-cyan/10 border border-cyan/30 text-cyan font-semibold text-sm hover:bg-cyan hover:text-white transition-colors"
              >
                📞 Call Us
              </a>
              <a
                href="/contact-news"
                className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-navy/10 border border-navy/20 text-navy font-semibold text-sm hover:bg-navy hover:text-white transition-colors"
              >
                ✉️ Contact Page
              </a>
            </div>
