...
function SectionHeading({ tag, title, subtitle, light = false, accentColor = 'bg-forest' }) {
  return (
    <div className="text-center mb-6">
      ...
      <div className={`mt-3 mx-auto w-16 h-1 rounded-full ${accentColor}`} />
    </div>
  );
}
...
export default function SocialActivities() {
  ...
  return (
    <main>
      ...
      <section
        id="social-hero"
        className="relative overflow-hidden hero-section bg-gradient-to-br from-forest via-[#0a4f2b] to-navy py-12 flex items-center"
      >
        {/* Logo watermark */}
        ...
        <div className="relative z-10 animate-fadeInUp max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-10 text-white text-center">
          {/* Eyebrow */}
          <div className="inline-flex items-center gap-2 bg-white/15 backdrop-blur border border-white/25 rounded-full px-4 py-1 text-xs sm:text-sm mb-3">
            ...
            Shivshakti GramVikas Pratishthan — सामाजिक उपक्रम
          </div>

          {/* Marathi tagline */}
          <h1 className="text-3xl sm:text-4xl md:text-5xl font-extrabold leading-tight mb-2 drop-shadow-lg"
              style={{ fontFamily: "'Noto Sans Devanagari', 'Inter', sans-serif" }}>
            एकच ध्यास... <span className="text-saffron">सर्वांगिण विकास</span>
          </h1>

          {/* English subtitle */}
          <p className="text-sm sm:text-base text-green-200 italic mb-1">
            "One Goal... All-Round Development"
          </p>
          <p className="text-xs sm:text-sm text-green-100 mb-5 max-w-xl mx-auto">
            Rooted in Faith, Committed to Community.
          </p>

          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            <a href="#tree-plantation"
              className="px-6 py-2.5 rounded-full bg-saffron hover:bg-orange-500 text-white font-bold text-sm shadow-lg shadow-orange-500/30 transition-all duration-200 hover:-translate-y-0.5">
              Explore Our Activities
            </a>
          </div>

          {/* 5 quick stats */}
          <div className="mt-6 grid grid-cols-3 sm:grid-cols-5 gap-2.5 max-w-2xl mx-auto">
            {(stats.length > 0 ? stats : DEFAULT_STATS).map(({ num, label, emoji }, i) => (
              <div key={i} className="bg-white/10 backdrop-blur rounded-xl p-2.5 border border-white/10">
                <p className="text-lg mb-0.5">{emoji}</p>
                <p className="text-lg font-extrabold text-white">{num}</p>
                <p className="text-[10px] text-green-200 mt-0.5 leading-tight">{label}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* ── 2. Tree Plantation ─────────────────────────────────────────────── */}
      <section id="tree-plantation" className="py-10 px-4 sm:px-6 lg:px-8 bg-gray-50 scroll-mt-28">
      ...
      <section id="varkari" className="py-10 px-4 sm:px-6 lg:px-8 bg-amber-50 scroll-mt-28">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="bg-white rounded-2xl border border-amber-100 shadow-sm p-6 sm:p-8 mb-10">
            ...
            <p className="text-gray-600 leading-relaxed">
              <strong className="text-navy">Shivshakti GramVikas Pratishthan</strong> actively supports the
      ...
      <section id="saptah" className="py-10 px-4 sm:px-6 lg:px-8 bg-[#1e0a2e] scroll-mt-28">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-12">
      ...
      <section id="other-programs" className="py-10 px-4 sm:px-6 lg:px-8 bg-white scroll-mt-28">
